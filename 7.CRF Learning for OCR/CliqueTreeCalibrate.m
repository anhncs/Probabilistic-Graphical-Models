...
function [P, logZ] = CliqueTreeCalibrate(P, isMax)
    ...
    if (doLogZ)
        %%% YOUR CODE HERE:
	%[cliqueOne,cliqueTwo] = find(P.edges);
	%cliqueOne(1);
	%cliqueTwo(1);
	%dodo=unnormalizedMessages(cliqueOne(1),cliqueTwo(1));
        %logZ = log(sum(dodo.val)) % remove this
	dodo = P.cliqueList(1);
	for ii=2:N
		dodo = FactorProduct(dodo,unnormalizedMessages(ii,1));
	end
	logZ = log(sum(dodo.val));
    else
        logZ = 0;
    end
